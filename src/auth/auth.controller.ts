...
import { FileNotImageException } from '../shared/exceptions/file-not-image.exception';
import { ApiFile } from 'src/shared/decorators/swagger.schema';
...
export class AuthController {
  ...
  async login(
    ...
    const safe: AuthLoginTransformer = await this.authService.mapLogin(user);
  ...
  async changePassword(
    ...
    if (newMatchPassword) {
      this.debuggerService.error("New password cant't same with old password", {
        class: 'AuthController',
        function: 'changePassword',
      });
