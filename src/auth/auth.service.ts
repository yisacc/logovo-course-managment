...
export class AuthService {
  ...
  async createPassword(password: string): Promise<IAuthPassword> {
    ...
    const salt: string = await this.helperService.hashingRandomSalt(saltLength);

    const passwordExpiredInDays: number = this.configService.get<number>(
      'auth.password.expiredInDay',
    );
    const passwordExpiredDate: Date =
      await this.helperService.dateTimeForwardInDays(passwordExpiredInDays);
    const passwordHash = await this.helperService.hashingBcrypt(password, salt);
    return {
      passwordHash,
      passwordExpiredDate,
      salt,
